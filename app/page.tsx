...
export default function ZeshuSuperApp() {
  ...
  // Constants
  const ZESHU_COINS_VAL = 50; // ₹50 discount
  const DONATION_VAL = 2; // ₹2 for charity
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen pb-40 font-sans antialiased text-gray-900">
      ...
      {/* 2. CATEGORY SCROLL (Blinkit Style) */}
      ...
      {/* 3. BILLS & RECHARGES (PhonePe Style - Per User Request) */}
                ...
                {cart.map((c, i) => (
                  <div key={i} className="flex justify-between items-center py-3 border-b border-gray-50 last:border-0">
                    ...
                    <div className="flex items-center bg-gray-100 rounded-lg">
                      ...
                      <button onClick={() => addToCart(c.item.item)} className="p-1"><Plus size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
