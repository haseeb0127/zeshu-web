          ...
          {activeTab === 'recharge' ? (
             <div className="bg-white rounded-[32px] shadow-xl border border-gray-100 max-w-2xl mx-auto overflow-hidden animate-in slide-in-from-bottom-4">
               <div className="flex overflow-x-auto bg-[#F8F9FC] p-3 gap-2 border-b border-gray-100 no-scrollbar">
                 {SERVICES.map((s) => (
                   <button key={s.id} onClick={() => { setActiveService(s.id); setSelectedOperator(''); setPlans([]); setFetchedBill(null); setRechargeNumber(''); setRechargeAmount(''); }} className={`flex items-center gap-2 px-5 py-3 rounded-[16px] whitespace-nowrap text-sm font-bold transition-all active:scale-95 ${activeService === s.id ? 'bg-white shadow-md text-[#111827]' : 'text-[#6B7280] hover:bg-[#E5E7EB]'}`}>
                     {s.icon} {s.label}
                   </button>
                 ))}
               </div>
               <div className="p-5 md:p-8 space-y-5">
                 ...
                 <button disabled={isLoading} onClick={isPlanBased ? fetchOffers : fetchBillDetails} className="w-full p-4 border-2 border-dashed border-[#C7D2FE] rounded-2xl text-[#4F46E5] bg-[#EEF2FF] hover:bg-[#E0E7FF] text-sm font-bold active:scale-[0.98]">{isLoading ? 'Fetching...' : 'Fetch Details'}</button>
                 {fetchedBill && (
                   <div className="bg-[#ECFDF5] border border-[#A7F3D0] p-6 rounded-2xl">
                     <div className="flex justify-between border-b border-[#D1FAE5] pb-3 mb-3"><span className="text-xs font-bold text-[#059669]">NAME</span><span className="font-bold">{fetchedBill.Name}</span></div>
                     <div className="flex justify-between"><span className="text-xs font-bold text-[#047857]">TOTAL DUE</span><span className="font-black text-xl">₹{fetchedBill.DueAmount}</span></div>
                   </div>
                 )}
                 {isPlanBased && plans.length > 0 && (
                   <div className="h-[300px] overflow-y-auto space-y-2 no-scrollbar p-1">
                     {plans.map((plan, idx) => (
                       <div key={idx} onClick={() => setRechargeAmount(String(plan.amount))} className={`bg-white p-4 rounded-xl border-2 cursor-pointer transition-all ${rechargeAmount === String(plan.amount) ? 'border-[#6366F1] bg-[#EEF2FF]/30' : 'border-gray-100 hover:border-gray-200'}`}>
                         <div className="flex justify-between font-black text-[#4F46E5]"><span>₹{plan.amount}</span><span className="text-[10px] bg-blue-100 px-2 rounded-md">{plan.validity}</span></div>
                         <p className="text-xs text-gray-500 mt-1 line-clamp-2">{plan.desc}</p>
                       </div>
                     ))}
                   </div>
                 )}
                 <input type="number" placeholder="Amount" className="w-full p-4 bg-[#F8F9FC] border border-gray-200 rounded-2xl font-black text-2xl outline-none" value={rechargeAmount} onChange={(e) => setRechargeAmount(e.target.value)} />
                 <button onClick={handleCartCheckout} className="w-full bg-gradient-to-r from-[#059669] to-[#047857] text-white py-5 rounded-2xl font-black text-lg active:scale-[0.98]">Proceed to Pay ₹{rechargeAmount || 0}</button>
               </div>
             </div>
          ) : (
            <>
