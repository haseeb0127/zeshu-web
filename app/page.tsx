...
export default function ZeshuSuperApp() {
  ...
  const [hasZeshuPass, setHasZeshuPass] = useState(false); // 🚀 NEW: SUBSCRIPTION STATE
  ...
  // 🚀 EXPANDED CATEGORIES INJECTED AUTOMATICALLY
  ...
  // ... (Keeping all your existing Supabase, MedPay, Auth, and Checkout logic exactly the same)
  ...
  // 🚀 NEW: CART MATH WITH SUBSCRIPTION LOGIC
  const itemTotal = cart.reduce((acc, curr) => acc + (curr.item.price * curr.qty), 0);
  const smallCartFee = (itemTotal > 0 && itemTotal < 199) ? 29 : 0; // Raised minimum threshold to ₹199 per your strategy
  const deliveryCharge = hasZeshuPass ? 0 : (itemTotal > 0 && itemTotal < 299) ? 30 : 0; // Free above ₹299 or with Pass
  const donationAmt = isDonating ? 1 : 0;
  const zeshuDiscount = useZeshuCoins ? Math.min(ZESHU_COINS_VAL, itemTotal) : 0; 
  const passFee = hasZeshuPass ? 99 : 0; // Zeshu Pass monthly fee added to cart if they toggle it
      ...
      <header className={`fixed top-0 w-full z-40 transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-2xl shadow-sm border-b border-gray-200/40' : 'bg-white border-b border-gray-100'}`}>
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-3 md:py-0 md:h-[88px] flex flex-col md:flex-row items-center justify-between gap-3 md:gap-8">
          ...
          <div className="hidden md:flex items-center gap-5 shrink-0">
            ...
            <button onClick={() => setIsCartOpen(true)} className="bg-gradient-to-b from-[#059669] to-[#047857] text-white px-5 py-3.5 rounded-[20px] flex items-center gap-3 font-bold text-sm min-w-[120px] justify-center active:scale-[0.96] shadow-lg shadow-green-600/20">
        ...
        <div className="flex-1 min-w-0 pb-32">
          {activeTab === 'recharge' ? (
             <div className="bg-white rounded-[32px] shadow-xl border border-gray-100 max-w-2xl mx-auto overflow-hidden animate-in slide-in-from-bottom-4">
               {/* Keep your existing Recharge code here... exactly as it was */}
               ...
               <div className="p-5 md:p-8 space-y-5">
                 {activeService === 'pharmacy' ? (
                   <div className="space-y-5">
                     ...
                     {/* ... Med Results logic remains here */}
                   </div>
                 ) : (
                   <>
                    <div>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full md:w-[460px] bg-[#F8F9FC] z-[70] shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              {/* 🚀 NEW: SUBSCRIPTION UPSELL (ZESHU PASS) */}
