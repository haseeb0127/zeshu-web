...
type Tab = "overview" | "orders" | "riders" | "vendors" | "products" | "customers";
...
export default function AdminDashboard() {
  ...
  return <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
    ...
    <main className="mx-auto max-w-7xl p-5">
      ...
      <nav className="mb-6 flex gap-2 overflow-x-auto">{(["overview", "orders", "riders", "vendors", "products", "customers"] as Tab[]).map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded-xl px-4 py-2 text-sm font-black capitalize ${tab === item ? "bg-violet-600 text-white" : "bg-white text-slate-500"}`}>{item}</button>)}</nav>
