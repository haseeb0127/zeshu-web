...
export default function AdminDashboard() {
  ...
  }), [supportConversations, supportStatusFilter, supportPriorityFilter]);
  ...
  return <div className="min-h-screen bg-[#f7f9f5] font-sans text-slate-900">
    ...
    <main className="mx-auto max-w-7xl p-5">
      ...
      <nav className="mb-6 flex gap-2 overflow-x-auto">{(["overview", "orders", "riders", "vendors", "products", "customers", "support"] as Tab[]).map((item) => <button key={item} onClick={() => { setTab(item); if (item !== "support") { supportDetailRequestRef.current += 1; supportDetailAbortRef.current?.abort(); setSelectedSupportConversation(null); } }} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-black capitalize ${tab === item ? "bg-[#087443] text-white" : "bg-white text-slate-500"}`}>{item === "support" && <MessageCircle size={15} aria-hidden="true" />}{item}</button>)}<Link href="/admin/marketing" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-[#087443]">Marketing</Link><Link href="/admin/gst" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-amber-700">GST workspace</Link><Link href="/admin/fulfillment" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-blue-700">Delivery & profit</Link><Link href="/admin/payments" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-violet-700">Payments</Link><Link href="/admin/partners" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-emerald-700">Partner failover</Link><Link href="/admin/drivers" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-amber-700">Driver verification</Link></nav>
