...
type SupportCategoryFilter = (typeof SUPPORT_CATEGORIES)[number];
...
export default function AdminDashboard() {
  ...
  const visibleSupportConversations = useMemo(() => supportConversations.filter((conversation) => {
  ...
  return <div className="min-h-screen bg-[#f7f9f5] font-sans text-slate-900">
    ...
    <main className="mx-auto max-w-7xl p-5">
      ...
      <nav className="mb-6 flex gap-2 overflow-x-auto">{(["overview", "orders", "riders", "vendors", "products", "customers", "support"] as Tab[]).map((item) => <button key={item} onClick={() => { setTab(item); if (item !== "support") { supportDetailRequestRef.current += 1; supportDetailAbortRef.current?.abort(); setSelectedSupportConversation(null); } }} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-black capitalize ${tab === item ? "bg-[#087443] text-white" : "bg-white text-slate-500"}`}>{item === "support" && <MessageCircle size={15} aria-hidden="true" />}{item}</button>)}<Link href="/admin/marketing" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-[#087443]">Marketing</Link><Link href="/admin/gst" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-amber-700">GST workspace</Link><Link href="/admin/fulfillment" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-blue-700">Delivery & profit</Link><Link href="/admin/payments" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-violet-700">Payments</Link><Link href="/admin/partners" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-emerald-700">Partner failover</Link></nav>
      ...
      {tab === "support" && <section className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(19,32,25,.06)]">
          ...
          {supportLoading && supportConversations.length === 0 ? <p className="mt-5 text-sm text-slate-500">Loading support inbox…</p> : <div className="mt-4 space-y-2">{visibleSupportConversations.length === 0 ? <p className="rounded-xl bg-[#f7f9f5] p-4 text-sm text-slate-500">No conversations in this view.</p> : visibleSupportConversations.map((conversation) => <button type="button" key={conversation.id} onClick={() => void openSupportConversation(conversation)} className={`w-full rounded-xl border p-3 text-left transition ${selectedSupportConversation?.id === conversation.id ? "border-emerald-400 bg-emerald-50" : conversation.support_priority === "URGENT" ? "border-red-300 bg-red-50" : conversation.support_priority === "HIGH" ? "border-orange-200 bg-orange-50" : conversation.status === "WAITING" ? "border-amber-200 bg-amber-50" : "border-slate-100 bg-white hover:bg-[#f7f9f5]"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-black text-slate-900">{conversation.subject || "Customer support"}</p><p className="mt-1 text-[11px] font-bold text-slate-500">Customer {String(conversation.user_id || "").slice(0, 8)} · {conversation.order_id ? `Order ${String(conversation.order_id).slice(0, 8)}` : "No order linked"}</p></div><div className="flex shrink-0 flex-col items-end gap-1">{conversation.support_category && conversation.support_category !== "Other" && <span className="max-w-32 truncate rounded-full bg-violet-50 px-2 py-1 text-[9px] font-black text-violet-700">{String(conversation.support_category).replace(" issue", "")}</span>}{conversation.support_priority && conversation.support_priority !== "NORMAL" && <span className={`rounded-full px-2 py-1 text-[9px] font-black ${conversation.support_priority === "URGENT" ? "bg-red-600 text-white" : conversation.support_priority === "HIGH" ? "bg-orange-500 text-white" : "bg-blue-100 text-blue-800"}`}>{conversation.support_priority}</span>}<span className={`rounded-full px-2 py-1 text-[10px] font-black ${conversation.status === "WAITING" ? "bg-amber-200 text-amber-900" : conversation.status === "RESOLVED" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{conversation.status === "WAITING" ? "WAITING" : conversation.status}</span></div></div>{conversation.last_message?.body && <p className="mt-2 truncate text-xs text-slate-600">{conversation.last_message.sender_role === "ADMIN" ? "You: " : ""}{conversation.last_message.body}</p>}<p className="mt-2 text-[10px] font-bold text-slate-400">Updated {conversation.updated_at ? new Date(conversation.updated_at).toLocaleString() : "—"}</p></button>)}</div>}
        </div>
