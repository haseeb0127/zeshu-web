    ...
    <main className="mx-auto max-w-7xl p-5">
      ...
      {tab === "support" && <section className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">{(["ALL", "WAITING", "OPEN", "RESOLVED"] as const).map((status) => <button type="button" key={status} onClick={() => setSupportStatusFilter(status)} className={`rounded-full px-3 py-1.5 text-[11px] font-black ${supportStatusFilter === status ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-600"}`}>{status === "WAITING" ? "Waiting for support" : status}</button>)}</div>
          {supportLoading && supportConversations.length === 0 ? <p className="mt-5 text-sm text-slate-500">Loading support inbox…</p> : <div className="mt-4 space-y-2">{supportConversations.filter((conversation) => supportStatusFilter === "ALL" || conversation.status === supportStatusFilter).length === 0 ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No conversations in this view.</p> : supportConversations.filter((conversation) => supportStatusFilter === "ALL" || conversation.status === supportStatusFilter).map((conversation) => <button type="button" key={conversation.id} onClick={() => void openSupportConversation(conversation)} className={`w-full rounded-xl border p-3 text-left transition ${selectedSupportConversation?.id === conversation.id ? "border-violet-400 bg-violet-50" : conversation.status === "WAITING" ? "border-amber-200 bg-amber-50" : "border-slate-100 bg-white hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-black text-slate-900">{conversation.subject || "Customer support"}</p><p className="mt-1 text-[11px] font-bold text-slate-500">Customer {String(conversation.user_id || "").slice(0, 8)} · {conversation.order_id ? `Order ${String(conversation.order_id).slice(0, 8)}` : "No order linked"}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${conversation.status === "WAITING" ? "bg-amber-200 text-amber-900" : conversation.status === "RESOLVED" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{conversation.status === "WAITING" ? "WAITING" : conversation.status}</span></div>{conversation.last_message?.body && <p className="mt-2 truncate text-xs text-slate-600">{conversation.last_message.sender_role === "ADMIN" ? "You: " : ""}{conversation.last_message.body}</p>}<p className="mt-2 text-[10px] font-bold text-slate-400">Updated {conversation.updated_at ? new Date(conversation.updated_at).toLocaleString() : "—"}</p></button>)}</div>}
        </div>
        <div className="rounded-2xl bg-white p-5 shadow-sm">{!selectedSupportConversation ? <div className="grid min-h-64 place-items-center text-center text-sm text-slate-500"><div><MessageCircle className="mx-auto mb-3 text-slate-300" size={32} /><p>Select a conversation to view the full thread.</p></div></div> : <><div className="flex flex-wrap items-start justify-between gap-3 border-b pb-4"><div><p className="font-black text-slate-900">{selectedSupportConversation.subject || "Customer support"}</p><p className="mt-1 text-xs font-bold text-slate-500">Customer {String(selectedSupportConversation.user_id || "").slice(0, 8)} · {selectedSupportConversation.order_id ? `Order ${String(selectedSupportConversation.order_id).slice(0, 8)}` : "No order linked"}</p></div><div className="flex gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black">{selectedSupportConversation.status}</span>{selectedSupportConversation.status !== "RESOLVED" && <button type="button" disabled={supportBusy} onClick={() => void resolveSupportConversation()} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-60">Resolve</button>}</div></div>{supportDetailLoading ? <p className="py-8 text-sm text-slate-500">Loading thread…</p> : <><div className="mt-4 max-h-[28rem] space-y-3 overflow-y-auto">{supportMessages.length === 0 ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No messages found.</p> : supportMessages.map((message) => <div key={message.id} className={`rounded-xl p-3 ${message.sender_role === "ADMIN" ? "ml-10 bg-violet-50" : "mr-10 bg-slate-50"}`}><p className="text-[10px] font-black uppercase tracking-wider text-slate-500">{message.sender_role === "ADMIN" ? "Zeshu support" : message.sender_role === "CUSTOMER" ? "Customer" : "Support"}</p><p className="mt-1 whitespace-pre-wrap break-words text-sm text-slate-800">{message.body}</p><p className="mt-2 text-[10px] text-slate-400">{message.created_at ? new Date(message.created_at).toLocaleString() : ""}</p></div>)}</div>{selectedSupportConversation.status !== "RESOLVED" && <div className="mt-4 border-t pt-4"><label htmlFor="admin-support-reply" className="sr-only">Support reply</label><textarea id="admin-support-reply" rows={3} maxLength={4000} value={supportReply} onChange={(event) => setSupportReply(event.target.value)} placeholder="Write a reply to the customer" className="w-full resize-none rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-violet-500" /><button type="button" disabled={supportBusy || !supportReply.trim()} onClick={() => void sendSupportReply()} className="mt-2 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50"><Send size={15} aria-hidden="true" />{supportBusy ? "Sending…" : "Send reply"}</button></div>}{selectedSupportConversation.status === "RESOLVED" && <p className="mt-4 rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800">Resolved conversations are read-only.</p>}</>}</>}
        </div>
      </section>}
    </main>
    {vendorModal && <Modal title="Add Vendor" close={() => setVendorModal(false)}><form onSubmit={(event) => void createVendor(event)} className="grid gap-3">{([["owner_id", "Existing Auth User UUID"], ["business_name", "Business Name"], ["address", "Address"], ["category", "Category (required)"], ["latitude", "Latitude (optional)"], ["longitude", "Longitude (optional)"], ["rating", "Rating 0-5 (optional)"]] as const).map(([key, label]) => <input key={key} required={["owner_id", "business_name", "address", "category"].includes(key)} type={key === "latitude" || key === "longitude" || key === "rating" ? "number" : "text"} step="any" placeholder={label} value={vendorForm[key]} onChange={(e) => setVendorForm({ ...vendorForm, [key]: e.target.value })} className="rounded-xl border p-3" />)}<label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={vendorForm.is_open} onChange={(e) => setVendorForm({ ...vendorForm, is_open: e.target.checked })} /> Store open</label><button disabled={saving} className="rounded-xl bg-violet-600 py-3 font-black text-white">{saving ? "Creating..." : "Create Vendor"}</button></form></Modal>}
    {riderModal && <Modal title="Add Rider" close={() => setRiderModal(false)}><form onSubmit={(event) => void createRider(event)} className="grid gap-3">{([["user_id", "Existing Auth User UUID"], ["full_name", "Full Name"], ["phone_number", "Phone Number"], ["vehicle_number", "Vehicle Number (optional)"]] as const).map(([key, label]) => <input key={key} required={key !== "vehicle_number"} placeholder={label} value={riderForm[key]} onChange={(e) => setRiderForm({ ...riderForm, [key]: e.target.value })} className="rounded-xl border p-3" />)}<label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={riderForm.is_active} onChange={(e) => setRiderForm({ ...riderForm, is_active: e.target.checked })} /> Start online</label><button disabled={saving} className="rounded-xl bg-violet-600 py-3 font-black text-white">{saving ? "Creating..." : "Create Rider"}</button></form></Modal>}
  </div>;
}
