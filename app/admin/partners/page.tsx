...
export default function AdminPartnersPage() {
  ...
  if (loading) return <main className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl rounded-2xl bg-white p-8 font-bold text-slate-500">Loading partner routing…</div></main>;

  return <main className="min-h-screen bg-slate-50 p-4 md:p-6">
    <div className="mx-auto max-w-7xl">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div><Link href="/admin/dashboard" className="text-xs font-black text-violet-700">← Admin dashboard</Link><h1 className="mt-1 text-3xl font-black text-slate-950">Partner routing &amp; failover</h1><p className="mt-1 max-w-3xl text-sm text-slate-500">Set primary and backup partners for 30-minute local delivery and resilient digital services. Lower priority number routes first.</p></div>
        <Link href="/admin/marketing" className="rounded-xl bg-white px-4 py-3 text-sm font-black text-violet-700 shadow-sm">Partner applications</Link>
      </div>
      ...
      <section className="mb-6 rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <label className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-3 text-xs font-black text-slate-600"><input type="checkbox" checked={form.auto_failover_enabled} onChange={(e)=>setForm({...form,auto_failover_enabled:e.target.checked})} /> Allow safe failover when rules permit</label>
        </div>
        ...
        <button disabled={busy === "new" || !form.name.trim()} onClick={() => void savePartner()} className="mt-4 rounded-xl bg-violet-600 px-5 py-3 text-sm font-black text-white disabled:opacity-50">{busy === "new" ? "Saving…" : "Add partner"}</button>
      </section>

      <div className="grid gap-5">
        {byCategory.map(({category,rows}) => <section key={category} className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          {rows.length > 0 && <div className="mt-4 grid gap-3 lg:grid-cols-2">{rows.map((partner,index)=><article key={partner.id} className="rounded-xl border border-slate-200 p-4">
            <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-wider text-violet-700">{index===0 ? "PRIMARY" : `BACKUP ${index}`}</p><h3 className="mt-1 font-black">{partner.name}</h3><p className="mt-1 text-xs text-slate-500">{partner.partner_kind} · priority {partner.priority}{partner.target_prep_minutes ? ` · prep target ${partner.target_prep_minutes}m` : ""}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-black ${partner.status === "ACTIVE" ? "bg-emerald-100 text-emerald-800" : partner.status === "DOWN" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-800"}`}>{partner.status}</span></div>
      ...
      <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm">
        ...
        {events.length === 0 ? <p className="mt-3 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No failover events yet.</p> : <div className="mt-3 space-y-2">{events.slice(0,20).map((event)=><div key={event.id} className="rounded-xl border border-slate-100 p-3 text-xs"><div className="flex justify-between gap-3"><span className="font-black">{event.category} · {event.outcome}</span><span className="text-slate-400">{new Date(event.created_at).toLocaleString()}</span></div><p className="mt-1 text-slate-600">{event.reason}</p></div>)}</div>}
      </section>
    </div>
  </main>;
}
