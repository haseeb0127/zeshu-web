...
export default function AdminMarketingPage() {
  ...
  if (loading) return <main className="min-h-screen bg-[#f7f9f5] p-6"><div className="mx-auto max-w-7xl rounded-2xl bg-white p-8 text-sm font-bold text-slate-500">Loading Marketing…</div></main>;

  return <main className="min-h-screen bg-[#f7f9f5] p-4 md:p-6">
    <div className="mx-auto max-w-7xl">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div><Link href="/admin/dashboard" className="text-xs font-black text-[#087443]">← Admin dashboard</Link><h1 className="mt-1 text-3xl font-black text-slate-950">Marketing</h1><p className="mt-1 text-sm text-slate-500">Clients, banners, sponsored products, scheduling and campaign reporting.</p></div>
        <div className="flex gap-2"><button onClick={() => setShowClient(true)} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#087443] shadow-[0_4px_16px_rgba(19,32,25,.06)]"><Plus size={16} className="mr-1 inline" /> Client</button><button onClick={() => { setForm(emptyCampaign); setShowCampaign(true); }} className="rounded-xl bg-[#087443] px-4 py-3 text-sm font-black text-white"><Plus size={16} className="mr-1 inline" /> Campaign</button></div>
      </div>
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[["Partner leads", partnerLeads.length], ["Campaigns", campaigns.length], ["Live", campaigns.filter((c) => campaignState(c) === "Live").length], ["Total clicks", campaigns.reduce((sum, c) => sum + Number(c.clicks || 0), 0)]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(19,32,25,.06)]"><p className="text-xs font-black uppercase tracking-wide text-slate-400">{label}</p><p className="mt-2 text-2xl font-black text-slate-950">{value}</p></div>)}
      </section>
      ...
      <section className="mb-6 rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(19,32,25,.06)]">
        ...
        {partnerLeads.length === 0 ? <p className="mt-4 rounded-xl bg-[#f7f9f5] p-4 text-sm text-slate-500">No partner applications yet.</p> : <div className="mt-4 grid gap-3 lg:grid-cols-2">{partnerLeads.map((lead) => <article key={lead.id} className="rounded-xl border border-slate-200 p-4">
          <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-wider text-[#087443]">{String(lead.partner_type || "").replaceAll("_"," ")}</p><h3 className="mt-1 font-black text-slate-950">{lead.business_name}</h3><p className="mt-1 text-xs text-slate-500">{lead.contact_person}{lead.city ? ` · ${lead.city}` : ""}</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{lead.status}</span></div>
          ...
          {lead.proposal && <p className="mt-3 rounded-lg bg-[#f7f9f5] p-3 text-xs leading-5 text-slate-600">{lead.proposal}</p>}
          <div className="mt-3 flex flex-wrap gap-2">{["CONTACTED","QUALIFIED","ONBOARDED","REJECTED"].map((status) => <button type="button" disabled={busy || lead.status === status} key={status} onClick={() => void setLeadStatus(lead,status)} className="rounded-lg bg-emerald-50 px-2.5 py-2 text-[10px] font-black text-[#087443] disabled:opacity-40">{status}</button>)}</div>
        </article>)}</div>}
      </section>

      {showClient && <section className="mb-6 rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(19,32,25,.06)]">
        ...
        ].map(([key,label]) => <label key={key} className="text-xs font-bold text-slate-600">{label}<input value={(clientForm as any)[key]} onChange={(e) => setClientForm({ ...clientForm, [key]: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-400" /></label>)}</div>
        <label className="mt-3 block text-xs font-bold text-slate-600">Notes<textarea value={clientForm.notes} onChange={(e) => setClientForm({ ...clientForm, notes: e.target.value })} rows={2} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-400" /></label>
        <button disabled={busy} onClick={() => void saveClient()} className="mt-4 rounded-xl bg-[#087443] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{busy ? "Saving…" : "Add client"}</button>
      </section>}

      {showCampaign && <section className="mb-6 rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(19,32,25,.06)]">
      ...
      <section className="rounded-2xl bg-white shadow-[0_4px_16px_rgba(19,32,25,.06)]">
