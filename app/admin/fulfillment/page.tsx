...
function ProductPanel({ product, saving, onChange, onSave }: { product: Row; saving: boolean; onChange: (key: string, value: any) => void; onSave: () => void }) {
  ...
  return <article className="rounded-3xl border border-[#dde7df] bg-white p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-xs font-black text-slate-600">Shipping class<select value={product.shipping_class || "STANDARD"} onChange={(event) => { const shippingClass = event.target.value; onChange("shipping_class", shippingClass); if (["COLD_CHAIN","LOCAL_ONLY"].includes(shippingClass)) onChange("nationwide_shipping_enabled", false); }} className="mt-1 block w-full rounded-xl border border-[#dde7df] px-3 py-2.5 text-sm"><option value="STANDARD">Standard</option><option value="FRAGILE">Fragile</option><option value="HEAVY">Heavy</option><option value="COLD_CHAIN">Cold chain</option><option value="LOCAL_ONLY">Local only</option></select></label>
