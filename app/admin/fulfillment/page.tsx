...
function ProductPanel({ product, saving, onChange, onSave }: { product: Row; saving: boolean; onChange: (key: string, value: any) => void; onSave: () => void }) {
  ...
  return <article className="rounded-3xl border border-[#dde7df] bg-white p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <label className="text-xs font-black text-slate-600">Delivery mode<select value={product.delivery_mode || "LOCAL_STANDARD"} onChange={(event) => { const mode = event.target.value; onChange("delivery_mode", mode); if (mode !== "INDIA_STANDARD") onChange("nationwide_shipping_enabled", false); if (mode !== "LOCAL_30_MIN") onChange("fresh_eligible", false); }} className="mt-1 block w-full rounded-xl border border-[#dde7df] px-3 py-2.5 text-sm"><option value="LOCAL_30_MIN">30-min Fresh</option><option value="LOCAL_STANDARD">Local Standard</option><option value="INDIA_STANDARD">India Standard</option></select></label>
      <label className="text-xs font-black text-slate-600">Shipping class<select value={product.shipping_class || "STANDARD"} onChange={(event) => { const shippingClass = event.target.value; onChange("shipping_class", shippingClass); if (shippingClass !== "STANDARD") onChange("nationwide_shipping_enabled", false); }} className="mt-1 block w-full rounded-xl border border-[#dde7df] px-3 py-2.5 text-sm"><option value="STANDARD">Standard</option><option value="FRAGILE">Fragile</option><option value="HEAVY">Heavy</option><option value="COLD_CHAIN">Cold chain</option><option value="LOCAL_ONLY">Local only</option></select></label>
      <SmallNumber label="Packed weight g" value={product.packed_weight_grams} onChange={(value) => onChange("packed_weight_grams", value)} />
      <SmallNumber label="Package length cm" value={product.package_length_cm} onChange={(value) => onChange("package_length_cm", value)} />
      <SmallNumber label="Package width cm" value={product.package_width_cm} onChange={(value) => onChange("package_width_cm", value)} />
      <SmallNumber label="Package height cm" value={product.package_height_cm} onChange={(value) => onChange("package_height_cm", value)} />
      <SmallNumber label="Handling minutes" value={product.handling_minutes} onChange={(value) => onChange("handling_minutes", value)} />
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      <Toggle label="India candidate" checked={product.nationwide_shipping_enabled === true} disabled={product.delivery_mode !== "INDIA_STANDARD" || product.requires_cold_chain === true || product.shipping_class !== "STANDARD"} onChange={(checked) => onChange("nationwide_shipping_enabled", checked)} />
    </div>
