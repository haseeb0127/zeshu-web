...
function ProductCard({ product, stockLoading, deleteLoading, onEdit, onToggle, onDelete }: { product: Product; stockLoading: boolean; deleteLoading: boolean; onEdit: () => void; onToggle: () => void; onDelete: () => void }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-[#dde7df] bg-white"><div className="flex gap-3 p-4"><div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-slate-800">{product.image_url && !imageFailed ? <img src={product.image_url} alt={product.name} onError={() => setImageFailed(true)} className="h-full w-full object-cover" /> : <ImageIcon aria-label="Product image unavailable" className="text-slate-500" />}</div><div className="min-w-0 flex-1"><h2 className="truncate font-black">{product.name}</h2><p className="mt-1 text-sm font-black text-[#087443]">₹{money(product.price)} <span className="font-medium text-slate-500">{product.unit || ""}</span></p><p className="mt-1 text-xs text-slate-500">{product.category || "Uncategorized"}{product.weight ? ` · ${product.weight}` : ""}</p></div></div><div className="flex flex-wrap gap-2 border-y border-[#dde7df] px-4 py-3 text-xs font-bold">{product.in_stock !== false ? <span className="rounded-full bg-emerald-500/10 px-2 py-1 text-emerald-700">In stock</span> : <span className="rounded-full bg-red-50 px-2 py-1 text-red-700">Out of stock</span>}{product.quantity !== null && product.quantity !== "" && <span className={`rounded-full px-2 py-1 ${lowStock ? "bg-amber-500/10 text-amber-200" : "bg-slate-800 text-slate-600"}`}>{lowStock ? "Low stock · " : "Quantity · "}{product.quantity}</span>}</div><div className="grid grid-cols-3 gap-2 p-3"><button onClick={onToggle} disabled={stockLoading} className="inline-flex items-center justify-center gap-1 rounded-xl bg-slate-800 px-2 py-2 text-xs font-black hover:bg-slate-700 disabled:opacity-60">{stockLoading ? <Loader2 className="animate-spin" size={15} /> : product.in_stock !== false ? <EyeOff size={15} /> : <Eye size={15} />}{product.in_stock !== false ? "Hide" : "Stock"}</button><button onClick={onEdit} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[#087443]/15 px-2 py-2 text-xs font-black text-[#087443] hover:bg-[#087443]/25"><Edit3 size={15} />Edit</button><button onClick={onDelete} disabled={deleteLoading} className="inline-flex items-center justify-center gap-1 rounded-xl bg-red-50 px-2 py-2 text-xs font-black text-red-700 hover:bg-red-100">{deleteLoading ? <Loader2 className="animate-spin" size={15} /> : <Trash2 size={15} />}Delete</button></div></article>;
}
