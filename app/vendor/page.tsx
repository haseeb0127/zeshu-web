...
function ProductForm({ form, editing, saving, onClose, onChange, onSubmit }: { form: typeof EMPTY_PRODUCT; editing: boolean; saving: boolean; onClose: () => void; onChange: React.Dispatch<React.SetStateAction<typeof EMPTY_PRODUCT>>; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const categoryField = <label className="block text-sm font-bold text-slate-300">Category <span className="text-red-300">*</span><select required value={form.category} onChange={(event) => onChange((current) => ({ ...current, category: event.target.value }))} className="mt-1.5 w-full rounded-xl border border-white/10 bg-slate-800 px-3 py-2.5 text-white outline-none focus:border-indigo-400"><option value="">Select customer category</option>{form.category && !knownCategory && <option value={form.category}>{form.category} (legacy)</option>}{VENDOR_PRODUCT_CATEGORIES.map((category) => <option key={category.id} value={category.id}>{category.label}</option>)}</select><span className="mt-1 block text-[10px] font-medium leading-4 text-slate-500">Use the closest standard category so customers can find this item and Zeshu can match backup vendors correctly.</span></label>;
