    ...
    <main className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-8">
      ...
      {tab === "dashboard" && <>
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[
          ["Products", products.length, ShoppingBag, "text-[#087443]"], ["In stock", inStockCount, CheckCircle2, "text-emerald-300"],
...
function OrderCard({ order, isTransitioning, onAdvance }: { order: Order; isTransitioning: boolean; onAdvance: () => void }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-[#dde7df] bg-white"><div className="flex items-start justify-between gap-3 border-b border-[#dde7df] bg-slate-800/60 p-4"><div><StatusBadge status={status} /><p className="mt-2 text-xs font-black text-slate-600">#{String(order.id).split("-")[0].toUpperCase()}</p><p className="mt-1 text-xs text-slate-500">{order.created_at ? new Date(order.created_at).toLocaleString() : ""}</p></div><div className="text-right"><p className="text-xl font-black">₹{money(order.total_paid)}</p><p className="text-xs text-slate-500">Delivery fee ₹{money(order.delivery_fee)}</p></div></div><div className="space-y-4 p-4"><div><p className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Items ({items.length})</p>{items.length ? <div className="space-y-2">{items.map((item, index) => <div key={index} className="flex justify-between rounded-xl bg-slate-800 px-3 py-2 text-sm"><span className="font-bold">{item?.qty ?? item?.quantity ?? 0} × {item?.item?.name || item?.name || "Product"}</span><span className="text-slate-500">₹{money(item?.item?.price ?? item?.price)}</span></div>)}</div> : <p className="text-sm text-slate-500">No readable item details.</p>}</div><div className="flex gap-2 text-sm text-slate-600"><MapPin className="mt-0.5 shrink-0 text-[#087443]" size={16} /><span>{order.delivery_address || "Address not provided"}</span></div></div><div className="border-t border-[#dde7df] p-4">{transition ? <button onClick={onAdvance} disabled={isTransitioning} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#087443] py-3 font-black text-white hover:bg-[#065F38] disabled:opacity-60">{isTransitioning ? <Loader2 className="animate-spin" size={18} /> : <Package size={18} />}{isTransitioning ? "Updating..." : transition.label}</button> : status === "READY_FOR_PICKUP" ? <p className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500/10 py-3 text-sm font-black text-emerald-300"><CheckCircle2 size={18} />Waiting for rider pickup</p> : <p className="text-center text-sm font-bold text-slate-500">No vendor action is available for this status.</p>}</div></article>;
}
...
function ProductCard({ product, stockLoading, deleteLoading, onEdit, onToggle, onDelete }: { product: Product; stockLoading: boolean; deleteLoading: boolean; onEdit: () => void; onToggle: () => void; onDelete: () => void }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-[#dde7df] bg-white"><div className="flex gap-3 p-4"><div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-slate-800">{product.image_url && !imageFailed ? <img src={product.image_url} alt={product.name} onError={() => setImageFailed(true)} className="h-full w-full object-cover" /> : <ImageIcon aria-label="Product image unavailable" className="text-slate-500" />}</div><div className="min-w-0 flex-1"><h2 className="truncate font-black">{product.name}</h2><p className="mt-1 text-sm font-black text-[#087443]">₹{money(product.price)} <span className="font-medium text-slate-500">{product.unit || ""}</span></p><p className="mt-1 text-xs text-slate-500">{product.category || "Uncategorized"}{product.weight ? ` · ${product.weight}` : ""}</p></div></div><div className="flex flex-wrap gap-2 border-y border-[#dde7df] px-4 py-3 text-xs font-bold">{product.in_stock !== false ? <span className="rounded-full bg-emerald-500/10 px-2 py-1 text-emerald-300">In stock</span> : <span className="rounded-full bg-red-500/10 px-2 py-1 text-red-300">Out of stock</span>}{product.quantity !== null && product.quantity !== "" && <span className={`rounded-full px-2 py-1 ${lowStock ? "bg-amber-500/10 text-amber-200" : "bg-slate-800 text-slate-600"}`}>{lowStock ? "Low stock · " : "Quantity · "}{product.quantity}</span>}</div><div className="grid grid-cols-3 gap-2 p-3"><button onClick={onToggle} disabled={stockLoading} className="inline-flex items-center justify-center gap-1 rounded-xl bg-slate-800 px-2 py-2 text-xs font-black hover:bg-slate-700 disabled:opacity-60">{stockLoading ? <Loader2 className="animate-spin" size={15} /> : product.in_stock !== false ? <EyeOff size={15} /> : <Eye size={15} />}{product.in_stock !== false ? "Hide" : "Stock"}</button><button onClick={onEdit} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[#087443]/15 px-2 py-2 text-xs font-black text-indigo-200 hover:bg-[#087443]/25"><Edit3 size={15} />Edit</button><button onClick={onDelete} disabled={deleteLoading} className="inline-flex items-center justify-center gap-1 rounded-xl bg-red-500/10 px-2 py-2 text-xs font-black text-red-200 hover:bg-red-500/20">{deleteLoading ? <Loader2 className="animate-spin" size={15} /> : <Trash2 size={15} />}Delete</button></div></article>;
}
