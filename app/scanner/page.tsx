...
export default function ScannerPage() {
  ...
  const [merchantName, setMerchantName] = useState(t('Scanned UPI recipient'));
  ...
  const handleSuccessfulScan = useCallback(async (text: string) => {
    ...
    setScanResult(text);
    setUpiId('');
    setMerchantName(t('Scanned UPI recipient'));
    try {
    ...
    const startCamera = async () => {
      try {
        ...
        if (!devices || devices.length === 0) {
          setCameraError(t('No camera detected on this device.'));
          return;
        }
        ...
        if (isMounted) {
          setCameraError(
            window.location.hostname !== 'localhost' && window.location.protocol === 'http:'
              ? t('Browsers block live cameras on HTTP IP addresses. Upload a QR image instead!')
              : t('Camera access denied. Check browser permissions.')
          );
        }
      }
    };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(t('Could not detect a valid QR code. Try another photo!'));
    }
  };
  ...
  const chooseContactRecipient = async () => {
    ...
    if (!('contacts' in navigator) || !('ContactsManager' in window)) {
      setContactMessage(t('Contact picker is not supported on this device. You can still scan a QR.'));
      return;
    }
    try {
      ...
      if (!phone) {
        setContactMessage(t('That contact does not have a usable mobile number.'));
        return;
      }
      ...
      setMerchantName(name || t('Selected contact'));
      setContactPhone(phone);
      setScanResult(null);
      setUpiId('');
      setAmount('');
      setIsScanning(false);
      setContactMessage(t('Contact selected. No money is sent; payments will be enabled only after verified UPI settlement is available.'));
    } catch {
      setContactMessage(t('Contact selection cancelled.'));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white flex flex-col font-sans selection:bg-emerald-500/30">
      ...
      <header className="p-4 flex items-center justify-between border-b border-slate-800">
        <button type="button" aria-label={t('Close scanner and return home')} onClick={() => router.push('/')} className="p-2 bg-slate-800 rounded-full active:scale-95 transition-transform">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative">
        {isScanning && (
          <div className="w-full max-w-md flex flex-col items-center">
            <h2 className="text-2xl font-black mb-1 tracking-tight text-center">{t('Scan any Shop QR')}</h2>
            <p className="text-slate-400 text-sm mb-2 text-center">{t('The QR scanner works across India.')}</p>
            <p className="text-amber-300/90 text-xs mb-6 text-center">{t('QR/contact payment and funded cashback will appear only after a verified payment and settlement integration is enabled.')}</p>
            ...
            <div className="w-full aspect-square bg-slate-900 rounded-3xl overflow-hidden border-2 border-emerald-500/50 shadow-[0_0_40px_rgba(8,116,67,0.22)] relative flex items-center justify-center">
              ...
              {cameraError && (
                <div className="absolute inset-0 bg-slate-900/95 p-6 flex flex-col items-center justify-center text-center z-20">
                  ...
                  <label className="bg-[#087443] text-white text-xs font-bold px-4 py-2.5 rounded-xl active:scale-95 transition-all shadow-lg shadow-emerald-500/30 cursor-pointer">{t('Upload a QR image')}<input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" /></label>
                </div>
              )}
              {!cameraError && (
            ...
            <div className="mt-6 grid w-full gap-3 sm:grid-cols-2">
              <label className="bg-[#18362a] hover:bg-[#214735] border border-emerald-900/60 text-slate-300 font-bold py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 text-sm">
                <ImageIcon size={18} className="text-emerald-400" /> {t('Upload QR')}
              ...
              <button type="button" onClick={() => void chooseContactRecipient()} className="bg-[#18362a] hover:bg-[#214735] border border-emerald-900/60 text-slate-300 font-bold py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95 text-sm">
                <BookUser size={18} className="text-emerald-400" /> {t('Choose contact')}
              </button>
            </div>
        ...
        {!isScanning && (
          <div className="w-full max-w-md flex flex-col items-center animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-2xl font-black mb-1 tracking-tight text-center">{recipientMode === 'CONTACT' ? merchantName : `${t('QR recipient')}: ${merchantName}`}</h2>
            ...
            <p className="mb-6 max-w-sm text-center text-xs font-bold leading-5 text-amber-300/90">{recipientMode === 'CONTACT' ? t('Contact selected for the future pay-to-contact flow. Zeshu has not sent or moved any money.') : t('QR details were read locally. Payment remains disabled until verified settlement is enabled.')}</p>

            <div className="w-full bg-slate-800/50 border border-slate-700 p-6 rounded-3xl mb-6">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">{t('Amount preview')}</label>
            ...
            <button type="button" disabled className="w-full rounded-2xl bg-slate-800 py-5 text-lg font-black text-slate-500">{recipientMode === 'CONTACT' ? t('Contact payments unavailable') : t('QR payments unavailable')}</button>{/*
            ...
            */}<button onClick={restartScanner} className="mt-4 text-xs text-slate-400 font-bold hover:text-white">{recipientMode === 'CONTACT' ? t('Scan a QR instead') : t('Scan Different QR')}</button>
          </div>
        )}
