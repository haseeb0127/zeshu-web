...
export default function ScannerPage() {
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        // Fetch cameras to see what's available
        ...
        // Pick back camera if available, otherwise pick the first camera
        ...
        if (isMounted) {
          setCameraError(
            window.location.hostname !== 'localhost' && window.location.protocol === 'http:'
              ? "Browsers block live cameras on HTTP IP addresses. Use localhost or upload a QR image below!"
  ...
  const handleSuccessfulScan = async (text: string) => {
    ...
    // Extract merchant name if standard UPI QR
    try {
  ...
  // 🖼️ FALLBACK: SCAN QR FROM FILE / IMAGE
  ...
  const processPayment = () => {
    ...
    setTimeout(() => {
      ...
      // Calculate random cashback (1% to 5%, capped at ₹20)
            ...
            <div className="w-full aspect-square bg-slate-900 rounded-3xl overflow-hidden border-2 border-indigo-500/50 shadow-[0_0_40px_rgba(99,102,241,0.2)] relative flex items-center justify-center">
              ...
              {cameraError && (
                <div className="absolute inset-0 bg-slate-900/95 p-6 flex flex-col items-center justify-center text-center z-20">
                  ...
                  <button 
                    onClick={() => handleSuccessfulScan("upi://pay?pa=shop@ybl&pn=Zeshu%20Mart&am=100")} 
                    className="bg-indigo-600 text-white text-xs font-bold px-4 py-2.5 rounded-xl active:scale-95 transition-all shadow-lg shadow-indigo-500/30"
                  >
        ...
        {!isScanning && !cashbackWon && (
          <div className="w-full max-w-md flex flex-col items-center animate-in fade-in zoom-in duration-300">
            ...
            <button 
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-[0.98] text-lg shadow-lg shadow-indigo-500/20"
            >
              {isProcessing ? (
                <><div className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full"></div> Processing Payment...</>
              ) : (
                `Pay ₹${amount || '0'} with Zeshu`
              )}
            </button>
            
            <button onClick={() => setIsScanning(true)} className="mt-4 text-xs text-slate-400 font-bold hover:text-white">
              Scan Different QR
            </button>
          </div>
        )}

        {/* STATE 3: CASHBACK WIN */}
        {cashbackWon !== null && (
          <div className="w-full max-w-md flex flex-col items-center text-center animate-in slide-in-from-bottom-10 fade-in duration-500">
            ...
            <p className="text-slate-400 font-medium mb-10">₹{amount} paid to {merchantName}</p>
            ...
            <button 
              onClick={() => router.push('/')}
              className="mt-10 w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-4 rounded-2xl active:scale-95 transition-all"
            >
              Back to Home
            </button>
          </div>
