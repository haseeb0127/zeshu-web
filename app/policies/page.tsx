...
const sections = [
  ...
  },
  {
    ...
    body: [
      ...
      'When the Zeshu Assistant AI feature is enabled, the text you send to that assistant and a limited recent support-chat context may be processed by Zeshu\'s configured AI service provider to generate a reply. The assistant request is configured not to be stored by the AI API where that option is supported. Do not send OTPs, passwords, card numbers, CVV, UPI PIN, API keys, access tokens, or other secrets. If the assistant cannot safely resolve an issue, the conversation may be transferred to Zeshu human support with the relevant question and assistant reply attached.',
