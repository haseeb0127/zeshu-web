...
import { Search, Zap, ChevronDown, ShoppingBag, User, X } from 'lucide-react';

// We define what information the Header needs to receive from the main page
  ...
  return (
    <header className={`fixed top-0 w-full z-40 transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-2xl shadow-sm border-b border-gray-200/40' : 'bg-white border-b border-gray-100'}`}>
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-3 md:py-0 md:h-[88px] flex flex-col md:flex-row items-center justify-between gap-3 md:gap-8">
        ...
        {/* Top Row on Mobile: Logo + Location + Cart Icon */}
        ...
        <div className="hidden md:flex items-center gap-5 shrink-0">
          <button onClick={() => user ? setIsAccountOpen(true) : setIsAuthModalOpen(true)} className="flex items-center gap-2 text-[#4B5563] font-extrabold text-sm active:scale-95">
