  ...
  return (
    <form onSubmit={submit} className="grid gap-4 rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm md:grid-cols-2">
      ...
      <label className="text-sm font-black text-slate-700">City / service area
        <input required maxLength={120} value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" placeholder="Jagtial / Karimnagar / Hyderabad" />
      </label>
      <label className="text-sm font-black text-slate-700">Vehicle
        <input maxLength={80} value={form.vehicle_type} onChange={(e) => setForm({ ...form, vehicle_type: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" placeholder="Bike / Auto / Car / Mini truck" />
      </label>
      <label className="text-sm font-black text-slate-700">Current registration
        <select value={form.registration_type} onChange={(e) => setForm({ ...form, registration_type: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-3">
          <option value="NO_VEHICLE">I do not have a vehicle yet</option>
          <option value="TRANSPORT">Transport / commercial registration</option>
          <option value="NON_TRANSPORT">Private / non-transport registration</option>
        </select>
      </label>
      <div className="rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-600">
        <p className="font-black text-slate-800">Privacy first</p>
        <p className="mt-1">Do not enter your driving-licence number, RC number, Aadhaar, PAN, bank details or OTP here. Zeshu will request documents only through a secured verification flow if onboarding opens.</p>
      </div>
      <label className="flex items-start gap-3 rounded-xl border border-slate-100 p-3 text-xs leading-5 text-slate-600">
        ...
        <span>I currently hold a valid driving licence for the vehicle/class I intend to use.</span>
      </label>
      <label className="flex items-start gap-3 rounded-xl border border-slate-100 p-3 text-xs leading-5 text-slate-600">
        ...
        <span>I can provide applicable RC, insurance, fitness, permit and PUC documents if required for my service.</span>
      </label>
      <label className="md:col-span-2 text-sm font-black text-slate-700">Anything else Zeshu should know?
        <textarea rows={4} maxLength={1200} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3 py-3" placeholder="Experience, preferred area, fleet size, EV, availability, etc." />
      </label>
      ...
      <label className="md:col-span-2 flex items-start gap-3 rounded-xl bg-emerald-50 p-3 text-xs leading-5 text-slate-700">
        ...
        <span>I confirm these details are accurate and Zeshu may contact me about onboarding. This is an interest form only; it does not mean I am approved to carry passengers, parcels or goods.</span>
      </label>
      ...
      <button disabled={busy || !form.consent} className="md:col-span-2 rounded-xl bg-[#087443] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{busy ? "Submitting…" : "Register my interest"}</button>
    </form>
  );
}
