    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full md:w-[460px] bg-[#F8F9FC] z-[70] shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
        <div className="bg-white px-6 py-5 flex justify-between items-center border-b">
          <h2 className="text-2xl font-black tracking-tighter">{t('My Cart')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {cart.map((c, i) => (
            <div key={i} className="bg-white p-4 rounded-2xl flex items-center gap-4 shadow-sm">
              <img src={c.item.image_url} className="w-16 h-16 object-contain" alt={c.item.name}/>
          ...
          <div className="bg-white p-6 rounded-[24px] shadow-sm space-y-4">
            <div className="flex justify-between text-[#4B5563]"><span>{t('Items total')}</span><span className="font-bold">₹{itemTotal}</span></div>
            <div className="flex justify-between text-[#059669]"><span>{t('Delivery charge')}</span><span className="font-black">{deliveryCharge === 0 ? t('FREE') : `₹${deliveryCharge}`}</span></div>
            <p className="text-xs font-medium text-[#587065]">{t('Free delivery on orders of ₹299 or more.')}</p>
            {smallCartFee > 0 && <div className="flex justify-between text-red-500 text-xs"><span>{t('Small cart fee')} ({t('ADD')} ₹{199-itemTotal} {t('to remove')})</span><span>₹{smallCartFee}</span></div>}
        ...
        <div className="bg-white p-6 border-t shadow-2xl">
          <button onClick={handleCartCheckout} className="w-full bg-gradient-to-r from-[#059669] to-[#047857] text-white font-bold py-4 rounded-2xl flex justify-between px-6 items-center shadow-lg shadow-green-600/30">
            <span>{t('Proceed to Pay')}</span><span>₹{finalCartTotal}</span>
          </button>
        </div>
      </div>
    </>
